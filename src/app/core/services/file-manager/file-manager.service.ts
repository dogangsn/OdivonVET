...
export class FileManagerService {
 ...
 getItemById(model:{id:string}|string):Observable<Item>{return defer(async()=>{const id=typeof model==='string'?model:model.id;const f=(await this.files.list()).find(x=>x.id===id);if(!f)throw new Error('Dosya bulunamadı.');const item=this.item(f);this._item.next(item);return item;});}
