...
2. Yönetim Auth'ta doğrudan üyelik için Confirm email seçeneğini kapatın; minimum şifre uzunluğunu 12 yapın. SMTP ve Turnstile CAPTCHA ayarlarını sağlayıcınıza göre yapılandırın. Ayrıntılar: registration-deployment.md. Site URL ve `/auth/registration` dönüş adresini yalnızca ürün alan adıyla sınırlandırın. Tarayıcı config dosyasında `turnstileSiteKey` tanımlayın.
